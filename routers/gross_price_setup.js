...
router.post("/save", auth, async(req, res) => {
    try {
        const { id_data, price } = req.body
        const product_data = await product.findById(id_data);
        product_data.gross_price = price;
        const newProduct = await product_data.save();
        res.json({
            data: newProduct,
            isChange: true,
            price: price
