...
router.post("/view", auth, async(req, res) => {
    try{
        const {name, email, mobile, password, status, warehouse, position, warehouse_cat, account_cat, sales_cat, sales_man_code} = req.body;
        const hash = await bcrypt.hash(password, 10)
        const data = new staff({name, email, mobile, status, warehouse, position, account_category:account_cat, type_of_acc_cat: sales_cat, sales_man_code : sales_man_code})
...
router.post("/view/:id", auth, async(req, res) => {
    try{
        ...
        const {name, email, mobile, password, status, warehouse, position, account_edit_cat, sales_cat2, sales_man_code2} = req.body;
